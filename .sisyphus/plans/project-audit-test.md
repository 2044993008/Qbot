...
- [ ] 15. Bot execute_tool CSRF fix + confirmation gate
...
- [ ] 16. Bot AI agent contract tests (mock OpenAI)

  **What to do**:
  - Problem: 2297-line bot route has ZERO tests for the AI agent logic
  - Strategy: Contract tests 鈥?mock OpenAI client, verify correct prompts are built and tool calls are parsed
  - Test cases:
    - System prompt includes SOUL + identity + memory (verify structure)
    - Tool call parsing: valid `[TOOL_CALL:...]` 鈫?correct tool + args
    - Tool call parsing: invalid JSON 鈫?handled gracefully
    - Preview generation: high-risk tools 鈫?preview object, not direct execution
    - Fallback: when LLM returns empty 鈫?friendly message
    - User lock: concurrent requests for same user 鈫?serialized (no race)
  - Use MSW or direct mock of `fetch` for OpenAI API calls
  - TDD: Write failing tests for each contract FIRST

  **Must NOT do**:
  - Do NOT test LLM output quality (not deterministic)
  - Do NOT call real OpenAI API in tests

  **Recommended Agent Profile**:
  > AI agent testing with mock infrastructure. Requires understanding of ReAct pattern.
  - **Category**: `deep`
    - Reason: Complex agent logic with tool calling, preview, and streaming; must mock external APIs
  - **Skills**: [`tdd-workflow`, `backend-patterns`]
    - `tdd-workflow`: Contract test design
    - `backend-patterns`: Mocking external API patterns

  **Parallelization**:
  - **Can Run In Parallel**: YES (with Task 17, Task 18, after Task 15)
  - **Parallel Group**: Wave 3 (with Task 17, Task 18, Task 19)
  - **Blocks**: None
  - **Blocked By**: Task 15 (bot security must be fixed first)

  **References**:
  - `src/app/api/bot/route.ts` 鈥?Full bot implementation (2297 lines)
  - `src/app/api/bot/route.ts:200-350` 鈥?System prompt building (SOUL + identity + memory)
  - `src/app/api/bot/route.ts:500-600` 鈥?Tool call parsing logic
  - `src/app/api/bot/route.ts:800-900` 鈥?Preview generation for high-risk tools
  - `src/app/api/bot/route.ts:100-200` 鈥?User lock mechanism

  **Acceptance Criteria**:
  - [ ] Test file `src/app/api/bot/agent-contract.test.ts` created
  - [ ] 鈮?8 test cases: prompt structure, tool parsing valid/invalid, preview generation, lock serialization, fallback on empty response
  - [ ] `vitest run src/app/api/bot/agent-contract.test.ts` 鈫?all PASS
  - [ ] Zero real API calls in test output

  **QA Scenarios**:

  ```
  Scenario: Tool call parsing extracts correct tool name and args
    Tool: Bash (vitest)
    Preconditions: Mock OpenAI to return [TOOL_CALL:{"tool":"search_messages","args":{"query":"hello"}}]
    Steps:
      1. Run agent with mock LLM response containing valid tool call
      2. Assert parsed tool name is "search_messages"
      3. Assert parsed args match {"query":"hello"}
    Expected Result: Tool call correctly extracted and parsed
    Failure Indicators: JSON parse error, wrong tool name
    Evidence: .sisyphus/evidence/task-16-tool-parse.txt
  ```

  **Commit**: YES
  - Message: `test(bot): add agent contract tests with mocked OpenAI client`
  - Files: `src/app/api/bot/agent-contract.test.ts`
  - Pre-commit: `pnpm test src/app/api/bot/agent-contract.test.ts`

- [ ] 17. TDD tests for scheduler.ts

  **What to do**:
  - Problem: `src/services/scheduler.ts` (332 lines) has ZERO tests 鈥?cron-based task execution
  - Create `src/services/scheduler.test.ts`
  - Test cases:
    - `reloadTasks()`: loads enabled tasks from DB, creates CronJob for each, skips disabled
    - `executeTask()`: reminder 鈫?sends message, send_message 鈫?inserts to conversation, post_moment 鈫?creates moment
    - `executeTask()`: error case 鈫?logs to `task_execution_logs` with status 'failed'
    - `startScheduler()`: creates interval, returns cleanup function
    - Cron expression parsing: valid cron 鈫?scheduled, invalid cron 鈫?handled
  - Mock: database client (Supabase), cron library, Socket.IO
  - TDD: Write failing tests FIRST, then verify

  **Must NOT do**:
  - Do NOT start real cron jobs in tests (mock the cron library)
  - Do NOT modify scheduler logic 鈥?only test the existing behavior

  **Recommended Agent Profile**:
  > Cron scheduler testing. Requires mock of time-based operations.
  - **Category**: `deep`
    - Reason: Time-based logic with cron scheduling and async task execution
  - **Skills**: [`tdd-workflow`]
    - `tdd-workflow`: Mocking time-based and scheduled operations

  **Parallelization**:
  - **Can Run In Parallel**: YES (with Task 16, Task 18, Task 19)
  - **Parallel Group**: Wave 3
  - **Blocks**: None
  - **Blocked By**: None (independent of other bot/task fixes)

  **References**:
  - `src/services/scheduler.ts` 鈥?Full scheduler implementation (332 lines)
  - `src/services/scheduler.ts:20-50` 鈥?reloadTasks function
  - `src/services/scheduler.ts:120-200` 鈥?executeTask function
  - `src/services/scheduler.ts:280-310` 鈥?startScheduler function
  - `src/storage/database/shared/schema.ts:scheduled_tasks,task_execution_logs` 鈥?DB schema

  **Acceptance Criteria**:
  - [ ] Test file `src/services/scheduler.test.ts` created
  - [ ] 鈮?10 test cases covering: reloadTasks, executeTask (all 3 types), error handling, cron parsing, start/shutdown
  - [ ] `vitest run src/services/scheduler.test.ts` 鈫?all PASS

  **QA Scenarios**:

  ```
  Scenario: executeTask creates task_execution_log on failure
    Tool: Bash (vitest)
    Preconditions: Mock DB INSERT to fail
    Steps:
      1. Call executeTask() with a reminder task, mock DB to throw
      2. Assert task_execution_logs INSERT called with status='failed'
      3. Assert error_message field populated
    Expected Result: Failed execution logged with error details
    Failure Indicators: Error silently swallowed, no log created
    Evidence: .sisyphus/evidence/task-17-scheduler-error-log.txt
  ```

  **Commit**: YES
  - Message: `test(scheduler): add comprehensive unit tests for cron scheduler service`
  - Files: `src/services/scheduler.test.ts`
  - Pre-commit: `pnpm test src/services/scheduler.test.ts`

- [ ] 18. Rate limiting for unprotected endpoints

  **What to do**:
  - Problem: moments like, moments comment, bot/stream have NO rate limiting
  - Add rate limits:
    - `POST /api/moments/like`: 30 requests/minute/user
    - `POST /api/moments/comment`: 20 requests/minute/user
    - `POST /api/bot/stream`: 20 requests/minute/user
  - Use existing `checkUserRateLimit()` from `rate-limit.ts`
  - TDD: Write test 鈥?exceeding limit 鈫?429 with Retry-After header

  **Must NOT do**:
  - Do NOT change existing rate limits (messages 30/min, login 5/min, upload 10/min)
  - Do NOT create new rate limit types 鈥?use existing infrastructure

  **Recommended Agent Profile**:
  > Rate limiting addition. Pattern-following task.
  - **Category**: `medium`
    - Reason: Applying existing rate limit pattern to 3 new endpoints
  - **Skills**: [`backend-patterns`]

  **Parallelization**:
  - **Can Run In Parallel**: YES (with Task 16, Task 17, Task 19)
  - **Parallel Group**: Wave 3
  - **Blocks**: None
  - **Blocked By**: None

  **References**:
  - `src/lib/rate-limit.ts:checkUserRateLimit()` 鈥?Existing rate limit function
  - `src/app/api/messages/route.ts:40-46` 鈥?Example of rate limit implementation pattern
  - `src/app/api/moments/like/route.ts` 鈥?Like endpoint (no rate limit)
  - `src/app/api/moments/comment/route.ts` 鈥?Comment endpoint (no rate limit)
  - `src/app/api/bot/stream/route.ts` 鈥?Bot stream endpoint (no rate limit)

  **Acceptance Criteria**:
  - [ ] All 3 endpoints return 429 when exceeding rate limit
  - [ ] Rate limit response includes `Retry-After` header
  - [ ] Test cases added to respective route test files
  - [ ] `vitest run` 鈫?affected tests PASS

  **QA Scenarios**:

  ```
  Scenario: Like rate limit enforced (30/min)
    Tool: Bash (curl)
    Preconditions: Clean rate limit state
    Steps:
      1. Send 31 POST /api/moments/like within 60 seconds
      2. Assert 31st request returns HTTP 429
      3. Assert Retry-After header is present
    Expected Result: HTTP 429 with Retry-After
    Failure Indicators: All 31 requests succeed (no rate limit)
    Evidence: .sisyphus/evidence/task-18-like-rate-limit.txt
  ```

  **Commit**: YES
  - Message: `security(rate-limit): add rate limiting to moments like/comment and bot stream endpoints`
  - Files: `src/app/api/moments/like/route.ts`, `src/app/api/moments/comment/route.ts`, `src/app/api/bot/stream/route.ts`
  - Pre-commit: `pnpm test`

- [ ] 19. Settings key whitelist + TDD
